...
config.pipeline.datamanager.dataparser.data = Path("cube")  # redundant but explicit
config.save_config()

trainer = config.setup(local_rank=0, world_size=1)
trainer.setup()
# Add this after trainer.setup() to inspect actual camera distances
import torch 
...
print(f"Transformed camera distances: min={dists.min():.3f} max={dists.max():.3f}")
trainer.train()